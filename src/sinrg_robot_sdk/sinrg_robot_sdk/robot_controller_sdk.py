...
class PacketFunction(enum.IntEnum):
    # (the control function realized through interface)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED(LED control)
    PACKET_FUNC_BUZZER = 2  #(buzzer control)
    PACKET_FUNC_MOTOR = 3  #(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM, 1-4(PWM servo control, numbered 1 to 4 from inside to outside on the board)
    PACKET_FUNC_BUS_SERVO = 5  # (bus servo control)
    PACKET_FUNC_KEY = 6  # (key obtain)
    PACKET_FUNC_IMU = 7  # IMU(IMU obtain)
    PACKET_FUNC_GAMEPAD = 8  # (handle obtain)
    PACKET_FUNC_SBUS = 9  # (remote control of model aircraft obtain)
    PACKET_FUNC_NONE = 10

class PacketReportKeyEvents(enum.IntEnum):
    # (the different states of key)
...
class Board:
    ...
    def get_button(self):
        # (Get the status of key1 and key2, return the key ID (1 for key1, 2 for key2) and the status (0 for pressed, 1 for released))
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # (get IMU data, return ax, ay, az, gx, gy, gz)
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
